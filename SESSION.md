...
- **Working tree: CLEAN.** All Stage 2/3 MuJoCo migration + Milestone 2 work, the gripper-joints work, the scene/object work, the gripper-controller-class work, and the `_move_arm()`/`ArmMotionClient` work are all committed on `fix/effort-mode-actuator-lock` (not merged to main, awaiting user review). Nothing uncommitted as of session end.

## What to do next session
1. **Stage 4 (grasping), FIRST recommended step: manually/scripted-test `GraspExecutor.pick()` against `demo_targets.get_target("grasp_object")` directly** (faster feedback than building the orchestration node first) — this is the first point where `grasp()`'s success can be verified against the real cube (not just free-air motion), the first real test of the pre-approach/approach/lift pose math already in `grasp_executor.py`, and where the flange-vs-fingertip pose calibration question (flagged when `"grasp_object"` was added to `demo_targets.py`) will actually surface and very likely need fixing.
2. Once `pick()` is validated directly: build the Stage 4 ROS2 orchestration node (joint-state subscriber + running the full grasp sequence as a real node, wrapping what's currently only exercised via ad-hoc test scripts).
...
4. Decide whether to merge `fix/effort-mode-actuator-lock` to main now (Milestone 1+2 closed) or keep iterating through Stage 4 first.
5. Root cause of the original Milestone 2 freeze remains open but non-blocking — do not spend time on it unless it recurs.
