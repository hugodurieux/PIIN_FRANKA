...
must be deactivated AND panda_effort_controller activated first (a single atomic
switch). panda_arm_controller's position PID otherwise holds the arm at its
start pose; panda_effort_controller (spawned --inactive) must be activated in the
SAME call, or nothing writes pinn_controller_node's effort commands to hardware
and the arm produces zero motion. Use ros2_ws/switch_to_effort.sh, or:
...
class MoveitPlanBridge(Node):
    ...
    def plan_and_handoff(self) -> None:
        ...
        # Publishing is asynchronous -- the DDS middleware needs to finish
        # discovering pinn_controller_node's subscriber and flush this message
        # before the node (and its publisher) is destroyed below. Without this
        # grace period, a freshly-started process can shut down before the
        # message ever leaves the middleware, silently dropping it (observed
        # live: two of three consecutive runs never reached
        # pinn_controller_node's _trajectory_cb at all).
        for _ in range(20):
